...
namespace HcomTestApplication
{
	public class ReceiveTargetData
	{
		...
		//Timer _readTimer;
		byte[] _prevRecvUnusedBytes = new byte[512];
		...
		public ReceiveTargetData(SerialPort serialPort, HostCommBuffer hostCommBuffer)
		{
			...
			if(_hostCommBuffer.HcomBufferInit(256) != HcomBufferReturn.HCOM_CIR_BUF_INIT_OK)
			{
		...
		void AddMessageToBuffer(byte[] buffer, int actualLength)
		{
			while(true)
			{
				var result = _hostCommBuffer.HcomBufferAddBytes(buffer, 0, actualLength);
		...
		void CheckForMessage(object state)
		{
			while(true)
			{
				...
				int packetLength;
				var result = _hostCommBuffer.HcomBufferGetNextPacket(packetBuffer, 512, out packetLength);
				switch(result)
				{
